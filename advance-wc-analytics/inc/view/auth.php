...
if (isset($_POST['awca_auth_submit']) && wp_verify_nonce($_POST['awca_nonce_header'], 'awca_auth_submit')) {
  ...
  if (isset($awca_auth_settings_save['agreement'])) {
      ...
      if (isset($awca_auth_settings_save['property_id'])) {
        if (strpos((string)$awca_auth_settings_save['property_id'], 'G') !== false) {
          ...
          if ($awca_auth_settings_save) {
            ...
            $errors .= 'Please complete Google Analytics Website Linking Process!<br>';
            $awca_auth_settings = $awca_auth_settings_save;
          }
        }
      } else {
        ...
        if (isset($awca_auth_settings_save['manual_tracking'])) {
          ...
          $errors .= 'Please confirm using manual tracking or use auto connect facility<br>';
          if (get_option('awca_auth_settings')) {
    ...
    $errors .= 'Please agree with privacy policy and terms of service of plugin!<br>';
    $awca_auth_settings = $awca_auth_settings_save;
  }
} else {
...
if (strlen($errors) > 0) {
  echo "<script>
            jQuery(document).ready(function(){
               M.toast({html: 'Please correct following Error', classes: 'rounded red', displayLength:6000});
  ...
  <form action="" method="POST">
    <div class="awca-col s12 top-mar">
      <div class="awca-col m6 s12">
        ...
        if ($auth_settings && ($awca_refresh_token_fail != 'yes')) { ?>
          <h5 class="left zero-mar"><?php _e('Authentication', 'advance-wc-analytics'); ?><span class="new badge green" data-badge-caption="<?php esc_attr_e('completed', 'advance-wc-analytics'); ?>"></span></h5>
        <?php } elseif ($auth_settings && ($awca_refresh_token_fail == 'yes')) { ?>
          <h5 class="left zero-mar"><?php _e('Authentication', 'advance-wc-analytics'); ?><span class="new badge yellow" data-badge-caption="<?php esc_attr_e('Reverification Required', 'advance-wc-analytics'); ?>"></span></h5>
        <?php } else { ?>
          <h5 class="left zero-mar"><?php _e('Authentication', 'advance-wc-analytics'); ?><span class="new badge red" data-badge-caption="<?php esc_attr_e('uncomplete', 'advance-wc-analytics'); ?>"></span></h5>
        <?php }
        ?>
      </div>
      <div class="awca-col m6 s12">
        <a class="waves-effect waves-light btn right upgrade-btn" style="margin-left:15px" href="<?php echo  awca_fs()->get_upgrade_url(); ?>"><?php _e('Upgrade to Pro!', 'advance-wc-analytics'); ?></a>
        <a class="waves-effect waves-light btn right" href="https://advancedwcanalytics.com/documentation" target="_blank"><i class="material-icons left">book</i><?php _e('Documentation', 'advance-wc-analytics'); ?></a>
      </div>
    </div>
    ...
    <div class="awca-row">
      <div class="awca-col s12 center-align">
        ...
        <div class="switch">
          <label>
            <b><?php _e('Auto Connect Method(Recommended)', 'advance-wc-analytics'); ?></b>
            <input <?php if (!($awca_auth_settings['property_id'] ?? false) && get_option('awca_access_token')) {
                      echo 'disabled';
                    } ?> id="awca_auth_settings[manual_tracking]" class="check_manual" name="awca_auth_settings[manual_tracking]" value="yes" type="checkbox" <?php checked(isset($awca_auth_settings['manual_tracking']) && $awca_auth_settings['manual_tracking']); ?>>
            <span class="lever"></span>
            <b><?php _e('Manual Connect Method', 'advance-wc-analytics'); ?></b>
          </label>
        </div>
        </p>
      </div>
      ...
      <div id="auto" class="awca-col s12">
        ...
        ?>
          <div class="notice notice-info top-mar">
            <p><b><?php _e('Please choose your property which you want to link with website and hit submit button.', 'advance-wc-analytics'); ?></b></p>
          </div>
        <?php } elseif (!get_option('awca_access_token')) { ?>
          <div class="input-field awca-col s12 center-align auto-connect">
            <div class="notice notice-info top-mar left-align">
              <b>
                <p class="left-align"> <?php _e('Please complete following requirements then click "Link your Google Analytics" button.', 'advance-wc-analytics'); ?></p>
              </b>
              <ol>
                <li> <?php _e('Need active Google Analytics account. If you not have one ', 'advance-wc-analytics'); ?><a target="_blank" href="https://marketingplatform.google.com/about/analytics/"><?php _e('please sign-up here.', 'advance-wc-analytics'); ?></a></li>
                <li> <?php _e('Need to have at least one active property for current website in Google Analytics Account. ', 'advance-wc-analytics'); ?></li>
              </ol>
            </div>
          </div>
          <div class="awca-row auto-connect">
            <div class="input-field awca-col s12 center-align">
              ...
              <span class="helper-text top-mar-10" data-error="wrong" data-success="right"><?php _e('Please link your Google Analyitcs Account for data collection.', 'advance-wc-analytics'); ?></span>
            </div>
          </div>
          <div class="input-field awca-col s12 center-align">
            <?php if (!($awca_auth_settings['property_id'] ?? false) && !($awca_auth_settings['tracking_id'] ?? false) && !get_option('awca_auth_settings')) { ?>
              <div class="notice notice-info top-mar manual-connect">
                <p><b> <?php _e('Please Enter your property-id / Data stream measurement id to link with website and hit submit button. ', 'advance-wc-analytics'); ?></b></p>
              </div>
            <?php } ?>
            <div class="awca-row manual-connect">
              <?php if (isset($awca_auth_settings['tracking_id']) && !empty($awca_auth_settings['tracking_id'])) {
                echo '<h6>' . $awca_auth_settings['tracking_id'] . '</h6>'; ?>
                <input placeholder="UA-XXXXX-X or G-XXXXXXXX" id="awca_auth_settings[tracking_id]" name="awca_auth_settings[tracking_id]" type="text" value="<?php if (isset($awca_auth_settings['tracking_id']) && !empty($awca_auth_settings['tracking_id'])) {
                                                                                                                                                                echo $awca_auth_settings['tracking_id'];
                                                                                                                                                              } ?>" class="tracking-id off">
                ...
                <div id="modal1" class="modal">
                  <div class="modal-content">
                    <h5><?php _e('Un-Link Google Analysis from website', 'advance-wc-analytics'); ?></h5>
                    <p><?php _e('Are you sure you wish to un-link Google Analysis from Website?', 'advance-wc-analytics'); ?></p>
                  </div>
                  <div class="modal-footer">
                    <a class="modal-close waves-effect waves-light btn AWCA-un-link"><?php _e('Un-Link Google Analytics', 'advance-wc-analytics'); ?></a>
                    <a class="modal-close waves-effect waves-green btn"><?php _e('Cancel', 'advance-wc-analytics'); ?></a>
                  </div>
                </div>
              <?php } else { ?>
                <div class="awca-col s12 center-align">
                  <div class="top-mar-30">
                    <div class="input-field awca-col m4 offset-m4 s12">
                      <input placeholder="G-XXXXXXXX" id="awca_auth_settings[tracking_id]" name="awca_auth_settings[tracking_id]" type="text" value="<?php if (isset($awca_auth_settings['tracking_id']) && !empty($awca_auth_settings['tracking_id'])) {
                                                                                                                                                        echo $awca_auth_settings['tracking_id'];
                                                                                                                                                      } ?>" class="tracking-id">
                      <span class="helper-text top-mar-10" data-error="wrong" data-success="right"><?php _e('Please put your Data stream measurement id from your GA4 here.', 'advance-wc-analytics'); ?></span>
                    </div>
                  </div>
                </div>
              <?php } ?>
            ...
            <div class="awca-col l8 offset-l2">
              <div class="awca-col s12 center-align">
                <?php if (isset($awca_auth_settings['property_id']) && !empty($awca_auth_settings['property_id'])) { ?>
                  ...
                  <input id="awca_auth_settings[property_id]" name="awca_auth_settings[property_id]" type="text" value="<?php if (isset($awca_auth_settings['property_id']) && !empty($awca_auth_settings['property_id'])) {
                                                                                                                          echo $awca_auth_settings['property_id'];
                                                                                                                        } ?>" class="property-id off">
                <?php } else { ?>
                  <div class="input-field awca-col m12 s12">
                    <?php
              ...
              <div id="modal1" class="modal">
                ...
                <div class="modal-footer">
                  <a class="modal-close waves-effect waves-light btn AWCA-un-link"><?php _e('Un-Link Google Analytics', 'advance-wc-analytics'); ?></a>
                  <a class="modal-close waves-effect waves-green btn AWCA-access-revoke"><?php _e('Remove Access for all My Websites', 'advance-wc-analytics'); ?></a>
                  <a class="modal-close waves-effect waves-green btn"><?php _e('Cancel', 'advance-wc-analytics'); ?></a>
                </div>
              </div>
            </div>
    ...
    <div class="awca-row">
      <div class="awca-col m8 offset-m2 center-align">
        <p>
          <label>
            <input type="checkbox" id="awca_auth_settings[agreement]" name="awca_auth_settings[agreement]" value="yes" <?php checked(isset($awca_auth_settings['agreement']) && ($awca_auth_settings['agreement'] == 'yes')); ?> />
            <span><?php _e('I agree with ', 'advance-wc-analytics'); ?><a href="https://advancedwcanalytics.com/privacy-policy/" target="_blank"><?php _e('privacy policy', 'advance-wc-analytics'); ?></a> <?php _e('and', 'advance-wc-analytics'); ?> <a href="https://advancedwcanalytics.com/terms-of-service/" target="_blank"><?php _e('terms of service', 'advance-wc-analytics'); ?></a><?php _e(' of AWCA Plugin.', 'advance-wc-analytics'); ?></span>
          </label>
        </p>
      </div>
    </div>
    ...
    <div class="awca-row center-align">
      <button class="btn waves-effect waves-light top-mar" type="submit" name="awca_auth_submit"><?php _e('Submit', 'advance-wc-analytics'); ?></button>
    </div>
